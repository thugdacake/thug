...
-- Comando para testar conexão com o Express
QBCore.Commands.Add('testarexpress', 'Testar conexão com servidor Express', {}, false, function(source, args)
    Debug("Comando de teste de conexão Express executado por jogador ID: " .. source)
    TriggerClientEvent('qb-laundry:client:testExpressConnection', source)
end)
...
AddEventHandler('onResourceStart', function(resourceName)
    if GetCurrentResourceName() == resourceName then
        ...
        -- Verificar se a tabela laundry_logs existe
        exports.oxmysql:execute("SHOW TABLES LIKE 'laundry_logs'", {}, function(result)
            if not result or #result == 0 then
                Debug("Tabela laundry_logs não encontrada, criando...")
                ...
                Debug("Tabela laundry_logs criada com sucesso")
            else
                Debug("Tabela laundry_logs encontrada")
            end
        end)
    end
end)

-- Evento para sincronizar dados com o Express (recebe dados do Express)
RegisterNetEvent('qb-laundry:server:syncExpressData')
AddEventHandler('qb-laundry:server:syncExpressData', function(token, expressData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    
    if not Player then 
        Debug("Tentativa de sincronização de dados com Express falhou: jogador inválido")
        return 
    end
    
    Debug("Dados recebidos do Express: " .. json.encode(expressData))
    
    -- Envia os dados para o cliente
    TriggerClientEvent('qb-laundry:client:syncExpressData', src, expressData)
end)

-- SISTEMA DE PROXY PARA EVITAR ERROS DE MIXED CONTENT
-- Criar um proxy HTTPS para o servidor Express
-- Isso resolve problemas de mixed content quando o FiveM carrega via HTTPS
-- mas o servidor Express roda em HTTP
RegisterNetEvent("qb-laundry:server:expressProxy")
AddEventHandler("qb-laundry:server:expressProxy", function(path, method, data, cb)
    local src = source
    local encodedData = json.encode(data or {})
    local validMethods = {GET = true, POST = true}
    
    if not validMethods[method:upper()] then
        Debug("Método HTTP inválido: " .. method)
        if cb then
            TriggerClientEvent("qb-laundry:client:expressProxyResult", src, 400, json.encode({error = "Método inválido"}), nil, cb)
        end
        return
    end
    
    Debug("Proxy Express: " .. method .. " " .. path)
    
    -- Faz a requisição ao Express como server-side
    PerformHttpRequest("http://localhost:8000" .. path, function(statusCode, responseText, headers)
        -- Retorna os resultados para o client
        if cb then
            TriggerClientEvent("qb-laundry:client:expressProxyResult", src, statusCode, responseText, headers, cb)
        end
    end, method:upper(), encodedData, {["Content-Type"] = "application/json"})
end)
